package ma.enova.repas.zynerator.util;

import java.io.Serializable;
import java.text.DecimalFormat;

public class FrenchNumberToWords implements Serializable {
    ...
    public static String chiffreToLettreFacture(double montant) {
        String toLettre = "";

        toLettre = NumberUtil.chiffreToLettre(NumberUtil.decomposerDecimal(montant)[0]) + " Dirhams ";
        if (NumberUtil.decomposerDecimal(montant)[1] != null && Double.valueOf(NumberUtil.decomposerDecimal(montant)[1]) > 0) {
            toLettre += NumberUtil.chiffreToLettre(NumberUtil.decomposerDecimal(montant)[1]) + " Centimes";
        }
        return toLettre;
    }
}
