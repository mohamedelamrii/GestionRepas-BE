...
import ma.enova.repas.zynerator.enumeration.ACTION_TYPE;
import ma.enova.repas.zynerator.exception.EntityNotFoundException;
import ma.enova.repas.zynerator.history.HistBusinessObject;
import ma.enova.repas.zynerator.history.HistCriteria;
...
import ma.enova.repas.zynerator.util.RefelexivityUtil;
import ma.enova.repas.zynerator.util.StringUtil;
import ma.enova.repas.zynerator.util.ListUtil;
import ma.enova.repas.zynerator.util.Utils;
...
*/
    ...
    public void deleteByIdIn(List<Long> ids) {
        if (ids != null) {
            if (ids.size() == 1) {
                dao.deleteById(ids.get(0));
            } else {
             //   dao.deleteByIdIn(ids); // plz add find by id apres delete !!!
                //constructAndSaveHistory(dto, ACTION_TYPE.DELETE); TO DO
            }
        }
    }
